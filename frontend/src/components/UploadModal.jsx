...
export default function UploadModal({ onClose, onSuccess, prefill = null }) {
  ...
  const inputClass = 'w-full px-3 py-2 border border-app rounded-md field-dark placeholder:text-dim focus:outline-none focus:ring-2 focus:ring-[var(--accent)]';
  /** Native selects ignore parent text color in some themes; force readable light-theme styling */
  const selectClass = `${inputClass} [&>option]:bg-[var(--bg-card)]`;
  const labelClass = 'block text-sm font-medium text-mid mb-1';
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      ...
      >
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={onClose} className="p-2 -m-1 rounded-md text-dim hover:text-dim" title="Close">
        ...
        {/* Mode tabs */}
        <div className="flex gap-2 mb-4 border-b border-app">
          <button
            type="button"
            onClick={() => { setMode('upload'); setError(''); }}
            className={`flex-1 py-2 text-sm font-medium rounded-t-md transition-colors ${mode === 'upload' ? 'text-[var(--accent)] border-b-2 border-[var(--accent)]' : 'text-dim hover:text-mid'}`}
          >
            Upload Screenshot
          </button>
          <button
            ...
            className={`flex-1 py-2 text-sm font-medium rounded-t-md transition-colors ${mode === 'manual' ? 'text-[var(--accent)] border-b-2 border-[var(--accent)]' : 'text-dim hover:text-mid'}`}
          >
            Add Manually
          </button>
        </div>
        ...
        {mode === 'upload' ? (
          <form onSubmit={handleUploadSubmit} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-mid mb-2">Investment Category</label>
              <select
          ...
          <form onSubmit={handleManualSubmit} className="space-y-4">
            ...
            <div>
              ...
              <label className="flex items-center gap-2 cursor-pointer mb-2">
                ...
                <span className="text-sm text-strong">Create new account</span>
              </label>
              {!createNewAccount ? (
                <select
            ...
            <div className="p-3 tip-box rounded-md">
              <p className="text-xs">
                {isAmountOnlyCategory
